...
data_top <- data[data$country %in% names(country_table_top), ]
country_app_year_table_top <- table(data_top$country, data_top$app_year)
print(country_app_year_table_top)
colors <- qualitative_hcl(nrow(country_app_year_table_top), palette = 'Dynamic')
barplot(country_app_year_table_top, beside = TRUE,
        main=title, xlab = "year", ylab = "count",
        legend=rownames(country_app_year_table_top),
        col=colors)
